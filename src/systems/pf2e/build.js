...
function extractHeightened(row) {
    const out = [];
...
function normalizeRow(row) {
    ...
    if (deities && deities.length)
        out.deities = deities;
    const bloodline = optionalTrim(row.Bloodline);
    if (bloodline)
        out.bloodline = bloodline;
    const heightened = extractHeightened(row);
    if (heightened.length)
        out.heightened = heightened;
    return out;
}
async function main() {
    ...
    for (const s of spells)
        for (const t of s.traditions)
            traditionSet.add(t);
    console.log(`wrote ${OUTPUT} — ${spells.length} spells (${traditionSet.size} traditions, ${traitSet.size} traits)`);
}
main().catch((err) => {
    console.error(err);
    process.exit(1);
